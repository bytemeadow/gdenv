...
[`gdenv`](https://github.com/bytemeadow/gdenv) is the best command-line tool to install and switch between multiple
versions of Godot.

This is the core library behind [`gdenv`](https://github.com/bytemeadow/gdenv).
It provides Godot version management, installation, project configuration parsing,
and addon synchronization as a reusable Rust library.

`gdenv-lib` is great for launching Godot from your Cargo project.
It uses the same `gdenv.toml` configuration file as `gdenv`.
It provides the `GodotRunner` builder to launch Godot with a single line of code.

# Run your Godot project with Cargo

The following steps will help you set up `cargo run` to run your Godot project with `gdenv-lib`.
...
[dependencies]
gdenv-lib = "1.2.0" # Update this to the latest version
...
[See the crate documentation on Docs.rs.](https://docs.rs/gdenv-lib/latest/gdenv_lib)

# License

`gdenv-lib` is distributed under the terms of both the MIT license and the Apache License (Version 2.0).
